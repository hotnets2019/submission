...
InboundMessage::InboundMessage(DelayPacketMetadata const &rx_packet, Environment const *environment)
        : msg_byte_len(rx_packet.message_len),
          from_peer{rx_packet.from_peer}, to_peer{rx_packet.to_peer}, msg_id_at_sender(rx_packet.message_id),
          msg_creation_time(rx_packet.message_creation_time), environment{environment} {
...
bool InboundMessage::append_packet_data(DelayPacketMetadata const &rx_packet) {
    ...
    for (auto it = begin(awaiting_data); it != end(awaiting_data);) {
        if (it->first > rx_packet.last_byte || it->second < rx_packet.first_byte) {
            it++;
            continue;
        }
