...
delay_transport::Analyzer::Analyzer(Environment *environment, double start_time, double report_delay)
        : environment{environment}, report_delay{report_delay}, timer{environment->create_timer()},
          sender_deliveries{0}, receiver_deliveries{0}, num_deadline_satisfied{0}, num_retransmission_drops{0},
          packets_sent{}, drop_count{}, num_new_messages{}, in_time_packets{}, out_of_time_packets{},
          before_time_packets{}, priority_used{}, expiration_remaining_slack_fraction{},
          drop_by_last_byte{}, drop_by_slack_fraction{}, expiration_remaining_data_fraction{} {
    timer->reschedule(std::max(0.0, start_time - environment->get_current_time()), [this] {
        cleanup();
        reschedule_report();
    });
}

void delay_transport::Analyzer::on_drop(delay_transport::DelayPacketMetadata const &packet,
    ...
    int const slack_bucket = std::max(0, std::min<int>(NUM_BUCKETS - 1, floor(
            NUM_BUCKETS * (packet.get_expiration_time() - packet.message_creation_time) /
            (double) (packet.message_expiration_time - packet.message_creation_time))));
    ...
    retransmission_indexes[get_group(packet.message_len)].push_back(packet.retransmission_idx);
...
void delay_transport::Analyzer::report() {
    auto const total_drop_count = std::accumulate(begin(drop_count), end(drop_count), 0);
    auto const avg_expiration_time =
            std::accumulate(begin(expiration_times), end(expiration_times), 0.0) / size(expiration_times);

    std::cerr << "STATS" << " ( " << environment->get_current_time() << " )\n";
    for (int group = 0; group < NUM_GROUPS; group++) {
        std::sort(begin(retransmission_indexes[group]), end(retransmission_indexes[group]));
        ...
        std::cerr << "successful (snd) deliveries: " << sender_deliveries[group] << " (in time: "
                  << num_deadline_satisfied[group] << ")\n";
        std::cerr << "successful (rcv) deliveries: " << receiver_deliveries[group] << "\n";
        std::cerr << "packets sent: " << packets_sent[group] << "\n";
        std::cerr << "drop count: " << drop_count[group] << "\n";
        if (!retransmission_indexes[group].empty()) {
            std::cerr << "median retransmission idx: "
                      << retransmission_indexes[group][retransmission_indexes[group].size() / 2] << "\n";
        }
        ...
        if (!violated_slacks[group].empty()) {
            auto const median_slack = violated_slacks[group][violated_slacks[group].size() / 2];
        ...
        if (!fraction_in_time[group].empty()) {
            std::cerr << std::accumulate(begin(fraction_in_time[group]), end(fraction_in_time[group]), 0.0) /
        ...
        if (!priority_used[group].empty()) {
            std::cerr << std::accumulate(begin(priority_used[group]), end(priority_used[group]), 0) /
    ...
    std::cerr << "== overall ==\n";
    std::cerr << "packets sent: " << std::accumulate(begin(packets_sent), end(packets_sent), 0) << "\n";
    std::cerr << "avg expiration time " << avg_expiration_time << "\n";
    if (!message_sizes.empty()) {
        std::sort(begin(message_sizes), end(message_sizes));
        auto const median_message_size = message_sizes[message_sizes.size() / 2];
        std::cerr << "drop median msg size: " << median_message_size << "\n";
    }


    std::cerr << "drop count: " << total_drop_count << " (retransmissions: " << num_retransmission_drops << ")\n";
...
void Analyzer::cleanup() {
    ...
    std::fill(begin(retransmission_indexes), end(retransmission_indexes), std::vector(0, 0));
    std::fill(begin(fraction_in_time), end(fraction_in_time), std::vector(0, 0.0));
    ...
    std::fill(begin(violated_slacks), end(violated_slacks), std::vector<double>{});
...
void Analyzer::on_sender_delivery(delay_transport::ApplicationMessageData const &app) {
...
void Analyzer::on_packet_sent(delay_transport::DelayPacketMetadata const &packet) {
    if (packet.get_expiration_time() > packet.message_expiration_time) {
        out_of_time_packets[get_group(packet.message_len)]++;
    } else if (packet.get_expiration_time() < environment->get_current_time()) {
        before_time_packets[get_group(packet.message_len)]++;
    } else {
        int const bucket = floor(NUM_BUCKETS * (packet.get_expiration_time() - packet.message_creation_time) /
                                 (packet.message_expiration_time - packet.message_creation_time));
        in_time_packets[get_group(packet.message_len)][bucket]++;
    }
    priority_used[get_group(packet.message_len)].push_back(packet.priority);
    packets_sent[get_group(packet.message_len)]++;
}

void Analyzer::on_new_message(delay_transport::ApplicationMessageData const &app) {
...
void Analyzer::on_message_timeout(int message_len, int bytes_delivered) {
    fraction_in_time[get_group(message_len)].push_back(bytes_delivered / (double) message_len);
}
...
void Analyzer::report_expiration(ApplicationMessageData const &app, double data_fraction, double slack_fraction) {
    ...
    int const data_bucket = std::min<int>(floor(data_fraction * NUM_BUCKETS), NUM_BUCKETS - 1);
    expiration_remaining_data_fraction[get_group(app.length_in_bytes)][data_bucket]++;
}
