...
public:
    ...
    void on_drop(delay_transport::DelayPacketMetadata const &packet, delay_transport::DropInfo const &info);
    ...
    void report_expiration(ApplicationMessageData const &app, double data_fraction, double slack_fraction) override;
...
private:
    ...
    std::array<std::array<int, NUM_BUCKETS>, NUM_GROUPS> expiration_remaining_slack_fraction;
    std::array<std::array<int, NUM_BUCKETS>, NUM_GROUPS> expiration_remaining_data_fraction;
