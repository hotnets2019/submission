...
public:
    explicit DelayTransportImpl(Environment *environment);

    ~DelayTransportImpl() override;

    auto on_new_app_message(ApplicationMessageData const &msg) -> message_id_t override;
    ...
    void on_packet_received(const DelayPacketMetadata &packet) override;

    DelayPacketMetadata
    create_packet_metadata(message_id_t msg_id, int first_byte, int last_byte, int bytes_remaining) override;

private:
    Analyzer *get_analyzer();

private:
    InboundMessage *find_inbound_message(DelayPacketMetadata const &packet,
...
private:
    ...
    std::unordered_map<message_id_t, std::list<std::unique_ptr<InboundMessage>>> incomplete_msgs;
