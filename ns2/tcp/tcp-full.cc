...
#include <delay_cc/FileDelayAssigner.h>
#include "ip.h"
#include "tcp-full.h"
#include "flags.h"
#include "random.h"
...
DDTcpAgent::need_send() {
 ...
 void DDTcpAgent::advance_bytes(int num_bytes) {
     deadline = delay_assigner_->assign_delay(num_bytes) * 1e6;
     FullTcpAgent::advance_bytes(num_bytes);
 }
 ...
 int DDTcpAgent::command(int argc, const char *const *argv) {
     if (argc == 3) {
